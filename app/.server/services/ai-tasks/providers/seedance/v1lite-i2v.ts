...
> = {
  ...
  calculateCredits(values) {
    ...
    if (resolution === "480p") secondCredits = 2;
    else if (resolution === "720p") secondCredits = 5;
    else if (resolution === "1080p") secondCredits = 10;
  ...
  async startTask(task) {
    ...
    try {
      const kie = getKieAI();
      const result = await kie.seeDance.v1LiteI2V.createTask({
  ...
  async queryTask(task) {
    ...
    try {
      const kie = getKieAI();
      const response = await kie.seeDance.v1LiteI2V.getTaskRecord(task_id);
...
/**
 * Seedance V1 Lite Provider 实现
 * 用于处理 Seedance AI 服务的图像转视频任务（Lite 版本）
 */
export const seeDanceV1LiteI2VProvider = new BaseProvider(config);
