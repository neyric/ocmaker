...
export const listCreditConsumptionsByUser = async (
  userId: CreditConsumption["user_id"],
  page = 1,
  pageSize = 50,
) => {
  const db = connectDB();
...
export const listConsumptionsByCreditRecordId = async (
  creditRecordId: CreditConsumption["credit_record_id"],
) => {
  const db = connectDB();
...
// 获取积分消耗记录详情
export const getCreditConsumptionsBySourceId = async (
  sourceId: CreditConsumption["source_id"],
) => {
  if (!sourceId) throw Error("Source ID is required");

  const db = connectDB();
  return await db.query.credit_consumptions.findMany({
    where: eq(schema.credit_consumptions.source_id, sourceId),
    with: { credit_record: true },
  });
};
